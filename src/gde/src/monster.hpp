...
class Monster: public CharacterBody3D {
	GDCLASS(Monster, CharacterBody3D)
...
#define MONSTER_HAS_VALID_TARGET(m) (MONSTER_IS_ACTIVE(m)&&MONSTER_IS_LOCKED(m))
		...
		short legal_monster_move(short monster_index, angle facing, Ref<Vector3> new_location);
		short legal_player_move(short monster_index, Ref<Vector3> new_location, world_distance *object_floor);
...
#define monsters_nearby(polygon_index) possible_intersecting_monsters(0, 0, (polygon_index), false)
		...
		void damage_monsters_in_radius(short primary_target_index, short aggressor_index, short aggressor_type,
			Vector3i *epicenter, short epicenter_polygon_index, real_t radius, struct damage_definition *damage, short projectile_index);
		void damage_monster(short monster_index, short aggressor_index, short aggressor_type, Vector3 *epicenter, struct damage_definition *damage, short projectile_index);
		...
		void set_type(MonsterType new_type);
		MonsterType get_type();
		uint16_t get_flags();
...
VARIANT_ENUM_CAST(Monster::MonsterType);
